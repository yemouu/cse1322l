...
public class Main {
  ...
  public static char[][] make_mirror(char[][] array) {
    ...
    for (int i = 0; i < array.length; i++)
      for (int j = 0; j < array[i].length; j++)
        if (array[i][j] == '(') r_array[i][array[i].length - 1 - j] = ')';
        else if (array[i][j] == ')') r_array[i][array[i].length - 1 - j] = '(';
        else if (array[i][j] == '/') r_array[i][array[i].length - 1 - j] = '\\';
        else if (array[i][j] == '\\') r_array[i][array[i].length - 1 - j] = '/';
        else r_array[i][array[i].length - 1 - j] = array[i][j];
  ...
  public static void main(String[] args) {
    ...
    for (int i = 0; i < ascii_art.length; i++) {
      for (int j = 0; j < ascii_art[i].length; j++) System.out.print(ascii_art[i][j]);
      System.out.println();
    }
    ...
    for (int i = 0; i < ascii_art_mirror.length; i++) {
      for (int j = 0; j < ascii_art_mirror[i].length; j++) System.out.print(ascii_art_mirror[i][j]);
      System.out.println();
    }
