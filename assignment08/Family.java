...
class Family {
  ...
  public ArrayList<Integer> getChildrenIDs(int id) {
    ...
    for (Person p : family) {
      if ((p.getParent1() == id) || (p.getParent2() == id)) childrenIDs.add(p.getID());
    }
  ...
  public int[] getParents(int ID) {
    ...
    for (Person p : family) {
      if (p.getID() == ID) {
        parentIDs[0] = p.getParent1();
        parentIDs[1] = p.getParent2();
      }
    }
  ...
  public void printParents(int childID, String relationship) {
    ...
    if (childID > -1) {
      ...
      for (int parent : parents) printParents(parent, parentRelationship(relationship));
    }
  }
  ...
  public void printChildren(int parentID, String relationship) {
    ...
    if (parentID > -1) {
      ...
      for (int child : children) printChildren(child, childRelationship(relationship));
    }
  }
}
